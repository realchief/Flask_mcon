...
unknown_placement_list = [
    "wFMvpasJrvvO_O4zh5FNbA==",
    "WKZIEqfBDHbq0B_7Xkz0Cg=="]

popads_placement_list = [
    "Qt9xZ5sayu46amCoRrsIIw==",
    "OKlfih5OwqQcW3kRAbr-qA=="]

popcash_placement_list = [
    "7Y36ZHPYtvfyn11zy5R4NQ==",
    "BpoQ2xYeqF950A4HqTT2JA=="]

exoclick_placement_list = [
"xwrl1gDaaSNrdL7ueAY9"]

trafficstars_placement_list = [
"dUAA3y5OGsDCunODuTNs"]

traffichaus_placement_list = [
"vvgyDm7Rs8dUQK123zSF"]

traffichunt_placement_list = [
"Cad7abiTEloFNpWt8qHR"]

propellorads_placement_list = [
"U75n0bcuSVjvGGu5o1Xc"]
...
if tracking_chance == 0:
    tracking_url = "http://trk.clickchaser.com/12da36fd-c4e8-4f52-b10f-4cdbb8e5c46a?message={}&headline={}&campaign={}&thumbnail={}&name={}&num_messages={}&badge={}&offer={}&placement={}&path={}&subscriber_key={}".format(
        message, headline, campaign, thumbnail, name, number_messages, badge, offer, placement, path, subscriber_key)
...
# MoboGamez
if tracking_chance == 0:
    tracking_url = "http://trk.clickchaser.com/edc563d7-34d9-4fbc-a6cb-e200733e3347?message={}&headline={}&campaign={}&thumbnail={}&name={}&num_messages={}&badge={}&offer={}&placement={}&path={}&subscriber_key={}".format(
        message, headline, campaign, thumbnail, name, number_messages, badge, offer, placement, path, subscriber_key)
...
traffic_sources = ['PopCash', 'PopAds', 'Unknown', 'ExoClick', 'TrafficStars', 'TrafficHaus', 'TrafficHunt',
'PropellorAds']

# Message Sending Loop
for source in traffic_sources:
    print(source)
    if source is 'PopCash':
        traffic_filter = {
            "field": "tag",
            "key": "sourcename_key",
            "relation": "=",
            "value": "PopCash"}
        placement = random.choice(popcash_placement_list)
    if source is 'PopAds':
        traffic_filter = {
            "field": "tag",
            "key": "sourcename_key",
            "relation": "=",
            "value": "PopAds"}
        placement = random.choice(popads_placement_list)
    if source is 'ExoClick':
        traffic_filter = {
            "field": "tag",
            "key": "sourcename_key",
            "relation": "=",
            "value": "ExoClick"}
        placement = random.choice(exoclick_placement_list)
    if source is 'TrafficStars':
        traffic_filter = {
            "field": "tag",
            "key": "sourcename_key",
            "relation": "=",
            "value": "TrafficStars"}
        placement = random.choice(trafficstars_placement_list)
    if source is 'TrafficHaus':
        traffic_filter = {
            "field": "tag",
            "key": "sourcename_key",
            "relation": "=",
            "value": "TrafficHaus"}
        placement = random.choice(traffichaus_placement_list)
    if source is 'TrafficHunt':
        traffic_filter = {
            "field": "tag",
            "key": "sourcename_key",
            "relation": "=",
            "value": "TrafficHunt"}
        placement = random.choice(traffichunt_placement_list)
    if source is 'PropellorAds':
        traffic_filter = {
            "field": "tag",
            "key": "sourcename_key",
            "relation": "=",
            "value": "PropellorAds"}
        placement = random.choice(propellorads_placement_list)  
    if source is 'Unknown':
        traffic_filter = {"field": "tag", "key": "sourcename_key", "relation": "not_exists"}
        placement = random.choice(unknown_placement_list)
    tracking_url = "http://trk.clickchaser.com/0faecec5-56de-4a85-acb9-49fa4902d071?message={}&headline={}&campaign={}&thumbnail={}&name={}&num_messages={}&badge={}&offer={}&placement={}&path={}&subscriber_key={}".format(
        message, headline, campaign, thumbnail, name, number_messages, badge, offer, placement, path, subscriber_key)
    logger.info('Sending Affiliate Link')
    logger.info('Tracking URL:')
...
requests.post(
    'https://hooks.slack.com/services/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',
    json={"text": "Push Notifications Paused"})
