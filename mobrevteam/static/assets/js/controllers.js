...
App.controller('MessagesCategories', ['$rootScope', '$scope', '$localStorage', '$window',
    function ($rootScope, $scope, $localStorage, $window) {
...
App.controller('MessagesGroups', ['$rootScope', '$scope', '$localStorage', '$window', '$state', 
    function ($rootScope, $scope, $localStorage, $window, $state) {
        // Init full DataTable, for more examples you can check out https://www.datatables.net/
        if (!$rootScope.groups)
            $rootScope.groups = [];
        ...
        $scope.edit = function (index) {
            $scope.tm_gp = {
                ...
                cat: $rootScope.groups[index].cat,
...
App.controller('AssignGroupToCampaign', ['$rootScope', '$scope', '$localStorage', '$window', '$state', '$stateParams',
    function ($rootScope, $scope, $localStorage, $window, $state, $stateParams) {
...
// Messages Controller
App.controller('Messages', ['$rootScope', '$scope', '$localStorage', '$window', '$state', '$stateParams',
    function ($rootScope, $scope, $localStorage, $window, $state, $stateParams) {
        ...
        $scope.update = function () {
            for (var i = 0; i < $rootScope.messages[$scope.current_id].messages.length; i ++ ){
                if ($rootScope.messages[$scope.current_id].messages[i].id == $scope.tm_message.id) {
                    $rootScope.messages[$scope.current_id].messages[i] = angular.copy($scope.tm_message)
                    break;
                }
            }
            $scope.tm_message = {
...
App.controller('MessagesMedia', ['$rootScope', '$scope', '$localStorage', '$window', '$state',
    function ($rootScope, $scope, $localStorage, $window, $state) {
...
App.controller('MessagesCampaigns', ['$rootScope', '$scope', '$localStorage', '$window',
    function ($rootScope, $scope, $localStorage, $window) {
        // Init full DataTable, for more examples you can check out https://www.datatables.net/
        if (!$rootScope.campaigns)
            $rootScope.campaigns = [];

        $scope.new_cp = $scope.tm_cp = {
            id: 0,
            name: null,
            locations: [],
        ...
        $scope.edit = function (index) {
            ...
            $scope.tm_cp = {
                id: $rootScope.campaigns[index].id,
                name: $rootScope.campaigns[index].name,
                all_day_notification: $rootScope.campaigns[index].all_day_notification,
                locations: locations,
                time_zone: $rootScope.campaigns[index].time_zone,
                from_time: {
                    hh: $rootScope.campaigns[index].from_time.hh,
                    mins: $rootScope.campaigns[index].from_time.mins,
                    m: $rootScope.campaigns[index].from_time.m
                },
                to_time: {
                    hh: $rootScope.campaigns[index].to_time.hh,
                    mins: $rootScope.campaigns[index].to_time.mins,
                    m: $rootScope.campaigns[index].to_time.m
                },
                one_message_per_mins: $rootScope.campaigns[index].one_message_per_mins,
                status: $rootScope.campaigns[index].status,
                stacking: $rootScope.campaigns[index].stacking
            };
            jQuery('#modal-popin').modal('show');
        }
        ...
        $scope.update = function () {
            for (var i = 0;i < $rootScope.campaigns.length; i ++) {
                if ($scope.tm_cp.id == $rootScope.campaigns[i].id) {
                    var locations = [];
                    for (var j = 0; j < $scope.tm_cp.locations.length; j ++) {
                        locations.push($scope.tm_cp.locations[j]);
                    }
                    $rootScope.campaigns[i] = {
                        id: $scope.tm_cp.id,
                        name: $scope.tm_cp.name,
                        all_day_notification: $scope.tm_cp.all_day_notification,
                        locations: locations,
                        time_zone: $scope.tm_cp.time_zone,
                        from_time: {
                            hh: $scope.tm_cp.from_time.hh,
                            mins: $scope.tm_cp.from_time.mins,
                            m: $scope.tm_cp.from_time.m
                        },
                        to_time: {
                            hh: $scope.tm_cp.to_time.hh,
                            mins: $scope.tm_cp.to_time.mins,
                            m: $scope.tm_cp.to_time.m
                            
                        },
                        one_message_per_mins: $scope.tm_cp.one_message_per_mins,
                        status: $scope.tm_cp.status,
                        stacking: $scope.tm_cp.stacking
                    }
                    break;
                }
            }
        ...
        $scope.add = function () {
            if ($scope.new_cp.name) {
                var id = $scope.max_idx + 1;
                $scope.max_idx = id;
                $rootScope.campaigns.push({
                    id: id,
                    name: $scope.new_cp.name,
                    all_day_notification: $scope.new_cp.all_day_notification,
                    locations: $scope.new_cp.locations,
                    time_zone: $scope.new_cp.time_zone,
                    from_time: {
                        hh: $scope.new_cp.from_time.hh,
                        mins: $scope.new_cp.from_time.mins,
                        m: $scope.new_cp.from_time.m,
                    },
                    to_time: {
                        hh: $scope.new_cp.to_time.hh,
                        mins: $scope.new_cp.to_time.mins,
                        m: $scope.new_cp.to_time.m
                    },
                    one_message_per_mins: $scope.new_cp.one_message_per_mins,
                    status: $scope.new_cp.status,
                    stacking: $scope.new_cp.stacking
                });
            }
            $scope.new_cp = {
                id: 0,
        ...
        $scope.is_in = function (l, locations) {
            for (var i = 0;i < locations.length; i++) {
                if (locations[i].id == l.id) return true;
            }
            return false;
        }
        ...
        $scope.cb_change = function (loc, key) {
            for (var i = 0;i < $scope[key].locations.length; i ++) {
                if (loc.id == $scope[key].locations[i].id) {
                    $scope[key].locations.splice(i, 1);
                    return;
                }
            }
            $scope[key].locations.push(loc);
        }
