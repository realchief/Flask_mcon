...
App.directive('jsYearCopy', function () {
    return {
        link: function (scope, element) {
            var gdate       = new Date();
            var baseYear    = element.html().length > 0 ? element.html() : gdate.getFullYear();

            if (parseInt(baseYear) >= gdate.getFullYear()) {
                element.text(gdate.getFullYear());
            } else {
                element.text(baseYear + '-' + gdate.getFullYear().toString().substr(2, 2));
            }
        }
    };
});
